...
enum EpubPackageDecoder {
    ...
    static func decode(archive: Archive) throws -> EpubPublication {
        ...
        do {
            packageData = try entryData(archive: archive, entry: packageResource.entry)
        } catch {
        ...
        let resolver = EpubResourceResolver(
            archive: archive,
            index: index,
            packagePath: packageResource.normalizedPath,
            package: package,
        ...
        guard !pages.isEmpty else {
            throw MankaiErrorCode.browseEpubNoReadableImages.makeError()
        }

        return EpubPublication(
            title: package.selectedTitle,
            credits: uniqueNonEmptyStrings(package.credits),
            ...
            coverPath: (cover ?? pages.first)?.entry.path,
            pagePaths: pages.map(\.entry.path)
        )
    }

    private static func loadPackageResource(
        archive: Archive,
        index: ArchiveIndex
        ...
        do {
            containerData = try entryData(archive: archive, entry: container.entry)
        } catch {
            throw MankaiErrorCode.browseEpubInvalidContainer.makeError(
                underlyingError: error
            )
    ...
    private static func encryptedResourcePaths(
        archive: Archive,
        index: ArchiveIndex
    ) throws -> Set<String> {
        ...
        let data: Data
        do {
            data = try entryData(archive: archive, entry: encryption.entry)
        } catch {
            throw MankaiErrorCode.browseEpubInvalidPackage.makeError(
                underlyingError: error
            )
...
private struct EpubResourceResolver {
    ...
    func coverResource() throws -> ArchiveResource? {
        if let coverItem = package.manifest.first(where: {
    ...
    private func imageResources(
        ...
        return try references.compactMap { reference in
            try rasterResource(
                reference: reference,
                relativeTo: resource.normalizedPath
            )
        }
    }

    private func rasterResource(
        reference: String,
        relativeTo basePath: String
    ) throws -> ArchiveResource? {
        ...
        guard let resource = index.resource(at: path) else {
            throw resourceNotFound(path: path)
        }
        try requireUnencrypted(resource, encryptedPaths: encryptedPaths)
        return resource
    }
...
private enum EpubPath {
    static func resolve(reference: String, relativeTo basePath: String) -> String? {
        ...
        if hasURIScheme(normalizedReference) {
            return nil
        }
        ...
        var components = basePath.replacingOccurrences(of: "\\", with: "/")
            ...
            .map(String.init)
        if !basePath.isEmpty, !components.isEmpty {
            components.removeLast()
        }

        for component in normalizedReference.split(
            separator: "/",
...
private struct ArchiveIndex {
    ...
    init(archive: Archive) {
        ...
        for entry in archive where entry.type == .file {
            guard let normalizedPath = EpubPath.normalizeArchivePath(entry.path) else {
                continue
            }
            let key = EpubPath.key(normalizedPath)
            if resources[key] == nil {
                resources[key] = ArchiveResource(
                    entry: entry,
    ...
    func resource(at path: String) -> ArchiveResource? {
        guard let normalizedPath = EpubPath.normalizeArchivePath(path) else {
            return nil
        }
...
private func requireUnencrypted(
    _ resource: ArchiveResource,
    encryptedPaths: Set<String>
) throws {
...
struct EpubPackageDocument {
    ...
    var modifiedValues: [String] {
        metaValues.compactMap {
            $0.property.lowercased() == "dcterms:modified" ? $0.value : nil
        }
    }
}
